...
receiver.listenBroadcast('image.uploaded', msg => {
  log.info('(Tweeter - Microservice)   Processing message now ...' + msg + ' from exchange image.uploaded');
  id = msg;

  timer.sleep(1000, () => {
    console.log('Message tweeted ' + id);
    return repository.updateData(id);
  })
    .then(item => {
      console.log('Sending event to browser...');
      console.log (item.value);
      sender.sendStompMessage('message.tweeted', JSON.stringify(item.value));
    })
    .catch(err => {
